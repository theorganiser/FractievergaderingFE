...
export async function POST(req: NextRequest) {
  ...
  // Verifieer toegangscode (server-side, niet zichtbaar in browser)
  const juisteCode = process.env.READER_PASSWORD
  if (!juisteCode) {
    return NextResponse.json({ fout: 'Server configuratiefout.' }, { status: 500 })
  }

  if (code !== juisteCode) {
    return NextResponse.json({ fout: 'Onjuiste toegangscode.' }, { status: 401 })
  }

  // Alles klopt — maak cookie aan
