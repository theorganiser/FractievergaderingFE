  ...
  return (
    <div>
      ...
      {gesorteerd.length === 0 ? (
        ...
        const actiefLijst = gesorteerd.filter(v => !v.datum || !isArchief(v.datum))
        const archiefLijst = gesorteerd.filter(v => v.datum && isArchief(v.datum))
        const renderVergadering = (v: Vergadering) => {
        ...
        return (
          <>
            {actiefLijst.map(v => renderVergadering(v))}
            {archiefLijst.length > 0 && (
              <div style={{ marginTop: '16px' }}>
                <button
                  onClick={() => setArchiefOpen(!archiefOpen)}
                  style={{ display: 'flex', alignItems: 'center', gap: '8px', background: 'none', border: '1px solid var(--rand)', borderRadius: '8px', padding: '10px 16px', cursor: 'pointer', fontSize: '13px', fontFamily: 'Arial', color: 'var(--tekst-zacht)', width: '100%', marginBottom: archiefOpen ? '10px' : '0' }}
                >
